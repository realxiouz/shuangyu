<template>
  <div class="contentBox">
    ...
    <div style="text-align: center">
      ...
      <el-button v-else size="mini" type="danger" @click="handleOnline">上 线</el-button>
    ...
    <el-dialog title="标签信息" center width="30%" :visible.sync="dialogVisible" :close-on-click-modal="false">
      <el-form ref="form" :model="formData" label-width="45px" size="mini">
...
export default {
  ...
  methods: {
    loadData() {
      this.$store
        ...
        .then(data => {
          ...
          this.selectFlags = [];
          if (
            this.formData.ownFlags &&
            this.formData.ownFlags != null &&
            this.formData.ownFlags.length > 0
          ) {
    ...
    handleSave() {
      ...
      if (this.editFlagFlag) {
        this.editFlagFlag = false;
        this.$store
          .dispatch("orderStaff/updateStaffOnline", {
            onlineFlags: JSON.stringify(this.formData.onlineFlags)
          })
          .then(() => {
            this.loadData();
          })
  ...
  mounted() {
    if (
      this.$store.state.loginInfo.staffId &&
      this.$store.state.loginInfo.staffId != null
    ) {
      this.loadData();
    }
  }
